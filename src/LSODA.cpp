...
c-----------------------------------------------------------------------
*/
void LSODA::lsoda(TOV* model, LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<double> &y, double *t,
...
void LSODA::stoda(
    const size_t neq, vector<double> &y,TOV* model, LSODA_ODE_SYSTEM_TYPE f)
{
    assert(neq + 1 == y.size());
    ...
    */
    while(1) {
        while(1) {
            ...
            correction(
                neq, y, model, f, &corflag, pnorm, &del, &delp, &told, &ncf, &rh, &m);
            if(corflag == 0)
                break;
            if(corflag == 1) {
...
*/
void LSODA::correction(const size_t neq, vector<double> &y, TOV* model, LSODA_ODE_SYSTEM_TYPE f,
 ...
 */
/* ----------------------------------------------------------------------------*/
void LSODA::lsoda_update(TOV* model, LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<double> &y,
    vector<double> &yout, double *t, const double tout, int *istate,
    double rtol, double atol)
{
    ...
    lsoda(model, f, neq, yout, t, tout, itask, istate, iopt, jt, iworks, rworks);
}
